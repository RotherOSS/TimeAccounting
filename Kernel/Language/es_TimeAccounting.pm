...
sub Data {
    ...
    $Self->{Translation}->{'You have to fill in start and end time or a time period.'}
        = 'Es necesario que proporcione una hora de inicio y una de finalización o un periodo.';
    ...
    $Self->{Translation}->{'Invalid format! Please enter a time with the format HH:MM.'}
        = '¡Formato no válido! Por favor introduzca las horas con el formato HH:MM.';
    ...
    $Self->{Translation}->{'Can\'t save settings, because a day has only 24 hours!'}
        = '¡No se puede guardar la configuración, porque un día sólo tiene 24 horas!';
